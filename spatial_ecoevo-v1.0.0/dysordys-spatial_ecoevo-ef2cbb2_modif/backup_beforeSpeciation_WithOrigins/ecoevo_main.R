...
if (length(clargs)>0) { # command-line arguments
  ...
  S <-  5# fifty species per trophic level
  ...
  MAX_S <- 10 # maximum amount of species allowed
}
...
organize_data <- function(dat, times) {
  ...
  dat %>%
    ...
    select(time,species,patch,m,n,o) %>%
    drop_na() %>%
    filter(o > 0) %>%
...
ic <- c(t(ninit),rep(0,(MAX_S-S)*L), t(muinit),rep(0,(MAX_S-S)*L),orinit,rep(0,MAX_S-S)) # merge initial conditions into a vector
...
pars <- list(SC=SC, L=L, rho=rho, kappa=kappa, a=a, eta=eta,
             ...
             tE=tE, d=d, mig=mig, model=model, MAX_S=MAX_S)
