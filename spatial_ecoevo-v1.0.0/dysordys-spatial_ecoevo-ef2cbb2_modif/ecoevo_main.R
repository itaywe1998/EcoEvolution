...
# ---------------------------- input parameters --------------------------------
#profvis({}) it is a nice idea for runtime-per-line profiler.
#When needed, all code must be between {} wrap
...
if (length(clargs)>0) { # command-line arguments
  ...
  MAX_S <- as.numeric(clargs[7]) # maximum amount of species allowed
} else { # sample input parameters, if no command line arguments are given
  S <-  1# fifty species per trophic level
  vbar <- 1e-3 # average genetic variance = 0.1 celsius squared
  ...
  outfile <- "output.data" # no output file; make plot instead
  MAX_S <- 40 # maximum amount of species allowed
}
Sys.setenv(S=S)
# --------------------------------functions ------------------------------------
...
organize_data <- function(dat, times) {
  dat <- dat %>%
  ...
  index <- 2
  # keep track of which column we are naming with this counter
  for (i in 1:MAX_S) {
    for (k in 1:L) { # name columns for densities
      names(dat)[index] <- paste0("n_", i, "_", k) # naming convention:
      index <- index + 1 # "type_species_patch" - type is either m (trait),
    } # or n (density)
  }
  for (i in 1:MAX_S) {
    for (k in 1:L) { # name columns for trait values
      names(dat)[index] <- paste0("m_", i, "_", k) # (same naming convention)
      index <- index + 1
    }
  }
  # for (k in 1:L-1){
  #   dat <- cbind(dat,dat %>% select(last_col(MAX_S-1):last_col()))
  # }
  # index <- 2*MAX_S*L+2
  # for (k in 1:L) {
  #   for (i in 1:MAX_S) { # name columns for trait values
  #     names(dat)[index] <- paste0("o_", i, "_", k) # (same naming convention)
  #     index <- index + 1
  #   }
  # }
  dat <- dat %>% select("time",contains('_'))
  dat %>%
    # normalize table by collapsing columns into a key-value column pair
    pivot_longer(cols=2:ncol(.), names_to="variable", values_to="v") %>%
    # split "variable" into value type (density, trait or origin), species, and patch
    ...
    #used to have o in the former implementation
    select(time,species,patch,m,n) %>%
    drop_na() %>%
   # filter(o > 0) %>%
    mutate(tl=ifelse(TRUE, "R", "C")) %>%
    # return tidy table
    return()
}
...
P <- 20 # size of parameters a,d,v,rho

# random- and trophic-dependent quantities
set.seed(1000*replicate+321) # set random seed for reproducibility
v <- runif(P, 0.5*vbar, 1.5*vbar) # resource genetic variances
d <- runif(P, 0.1*dbar, 10.0*dbar) # resource dispersal rates
rho <- runif(P, 0.9, 1.1) # resource growth-tolerance tradeoff parameter
a <- matrix(0, P, P) # initialize full competition matrix (resources+consumers)
aP <- matrix(runif(P*P, 0.15*0.5, 0.15*1.5), P, P) # resource comp coeffs
diag(aP) <- runif(P, 0.2*0.5, 0.2*1.5) # resource intraspecific comp coeffs
a[1:P,1:P] <- aP # top left block: resources
...
vmat <- matrix(rep(v, L), P, L) # genetic variances at each patch
...
y <- 100
tstart <- -400*y # starting time (relative to start of climate change at t = 0)
tE <- 200*y*5000 # time at which climate change stops (assuming it starts at t = 0)
tend <- tE+250*y # time at which integration ends
split <- 1.0 #split ratio between ancestor and descendant growth rate at Speciation
speth <- 1e-1 # Density threshold for the Speciation Event, in order to prevent 
#re-occurence of the same event -> S explodes around few "real" events.
...
orinit <- round(seq(1,L, l=SR)) # originial cell
# initial temperatures
Tempinit <- Temp(seq(from=0, to=1, l=L), 0, tE, Cmax, Cmin, Tmax, Tmin)
for (i in 1:SR){
  ninit[i,] <- exp(-(muinit[i,1]-Tempinit)^2/(2*2^2))
  orinit[i] <- which.max(ninit[i,]) #just making sure
} 
Sys.setenv(origins=vec2String(orinit))
...
ic <- c(t(ninit),rep(0,(MAX_S-S)*L), t(muinit),rep(0,(MAX_S-S)*L)) # merge initial conditions into a vector
# coerce parameters into a list
step <- 1 # dt for the eqs inside ode, passed as hini to ode (fixed time-step)
iteration<-FALSE
mt <- "iteration"
if (mt == "iteration") {
  iteration<-TRUE
}
pars <- list(SC=SC, L=L, rho=rho, kappa=kappa, a=a, eta=eta,
             eps=eps, W=W, venv=venv, vmat=vmat, V=V, nmin=nmin, aw=aw, bw=bw,
             Tmax=Tmax, Tmin=Tmin, Th=Th, arate=arate, Cmax=Cmax, Cmin=Cmin,
             tE=tE, d=d, mig=mig, model=model, P=P, tstart=tstart,MAX_S=MAX_S,
             tend=tend,split=split,step=step,iteration=iteration,speth=speth)


# --------------------------- integrate ODEs -----------------------------------
before_step <- 2 * y
before_cc <- tryCatch({
  ode(y=ic, times=seq(tstart, 0, by=before_step), func=eqs, parms=pars,
      method=mt, hini =step)}, error=function(err){
                   print(paste("Oh What Happened? : ",err))}) # integrate ODEs before climate change starts
...
before_cc <- before_cc %>% # put before-climate-change solution into tidy tibble:
  organize_data(times=seq(from=tstart, to=0, by=before_step)) %>%
  filter(time!=0) # remove time point 0 (will be starting point of during_cc)
print(Sys.time()-start)
during_step <- 0.5 * y *5000
during_cc <-tryCatch({
                   ode(y=ic, times=seq(0, tE, by=during_step), func=eqs, parms=pars,
                       method=mt, hini =step)}, error=function(err){
                         print(paste("Oh What Happened? : ",err))})  # integrate from start to end of climate change
...
during_cc <- during_cc %>% # put during-climate-change solution into tidy tibble:
  organize_data(times=seq(from=0, to=tE, by=during_step)) %>%
  filter(time!=tE) # remove time point tE (will be starting point of after_cc)

after_step <- 20 * y 
after_cc <- tryCatch({
  ode(y=ic, times=seq(tE, tend, by=after_step), func=eqs, parms=pars,
      method=mt, hini =step)}, error=function(err){
        print(paste("Oh What Happened? : ",err))}) %>% # integrate from end of climate change to end
  # put after-climate-change solution into tidy tibble:
  organize_data(times=seq(from=tE, to=tend, by=after_step))
...
# --------------------------- generate output ----------------------------------
dat <- dat[order(dat$time,dat$patch),] # original convention
