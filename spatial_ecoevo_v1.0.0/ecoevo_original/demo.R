...
large_tstart <- -1e6 * y
plot1 <- plot_timeseries(large_dat %>% filter(time %in% c(large_tstart, large_tstart-large_tstart/200, 0 , final_time)))+ggtitle("Long Adaptation Period")
...
small_tstart <- -1e3 * y
plot2 <- plot_timeseries(small_dat %>% filter(time %in% c(small_tstart, small_tstart-small_tstart/200,0, final_time)))+ggtitle("Short Adaptation Period")
