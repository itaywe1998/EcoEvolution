...
List eqs(double time, NumericVector state, List pars) {
  ...
  int S=atoi(getenv("S"));
  int L=pars["L"],MAX_S=pars["MAX_S"], SR = S;
  ...
  /*std::cout<<"S now is "<<S;
  std::cout<<std::endl;*/
  NumericVector origins(S);
  NumericVector dvdt(2*MAX_S*L+MAX_S);
  NumericVector x(L), T(L); 
  //dvdt is the next state, the first state is ic ...
  // Assign state variables into matrices n and m; calculate local temperatures
  for (i=0; i<S; i++) {
    origins[i] = state[2*MAX_S*L + i];
    for (k=0; k<L; k++) {
      x[k]=k/((double)L-1.0); // Patch k's distance from pole
      n(i,k)=state[k+i*L]; // Density of species i in patch k
      if (n(i,k)<1.0e-10) n(i,k)=0.0; // Extinction threshold
      m(i,k)=state[MAX_S*L+k+i*L]; // Trait mean of species i in patch k
    }
  }
  ...
  /*for (i=0; i<S; i++) {
    int o = origins[i];
    for (k=0; k<L; k++) {
      if (abs(m(i,k)-T[k]) < abs(m(i,k)-T[o])) //if closer in trait to local env
        newS++;
    }
  }*/
  //NumericVector new_origins (newS);
  //for (i=0; i<S; i++) new_origins[i]=origins[i];
  /*for (i=0; i<newS; i++) {
    int o = origins(i);
    for (k=1; k<L; k++) {
      if (abs(m(i,k)-T(k)) < abs(m(i,k)-T(o))) //if closer in trait to local env
  ...
  for (k=0; k<L; k++) {
    ...
    for (i=0; i<S; i++) {
      ...
      dvdt[k+i*L]=(n(i,k)*b+sumgr)*smoothstep(n(i,k)/1.0e-6)+summig;
      dvdt[MAX_S*L+k+i*L]=h2*(g-bsumgr+bsummig);
...
//  std::cout <<"dvdt size is "<<dvdt.size()<<std::endl;
 // if ((int)time%1000 == 0) setenv("S",std::to_string(S+1).c_str(),1);
